...
-- Script này XÓA TẤT CẢ DỮ LIỆU và tạo lại từ đầu
-- CHỈ CHẠY KHI MUỐN RESET DATABASE HOÀN TOÀN
-- =============================================

USE master;
GO

-- Xóa database nếu đã tồn tại (CẢNH BÁO: Mất tất cả dữ liệu!)
IF EXISTS (SELECT name FROM sys.databases WHERE name = 'SWP391_Tool')
BEGIN
    ALTER DATABASE SWP391_Tool SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE SWP391_Tool;
    PRINT 'Database SWP391_Tool dropped';
END
GO

-- Tạo database mới
CREATE DATABASE SWP391_Tool;
PRINT 'Database SWP391_Tool created';
GO
...
PRINT 'Foreign key FK_Courses_Lecturer created';
...
PRINT 'Table ProjectMembers created';
...
PRINT 'Table ProjectConfigs created';
...
PRINT 'Table ProjectRepositories created';
...
PRINT 'Table ProjectSprints created';
...
PRINT 'Table JiraIssues created';
...
PRINT 'Table GitHubCommits created';
...
PRINT 'Table AnomalyReports created';
...
PRINT 'Table SRSDocuments created';
